    ...
    <q-card class="q-dialog-plugin" style="min-width: 50vw">
      ...
      <q-form @submit.prevent="submit">
        ...
        <q-card-section v-if="mode === 'script'" class="q-pt-none">
          <tactical-dropdown
            v-model="state.args"
            label="Arguments de script (faites Entrée après chaque argument saisi)"
            filled
            use-input
            multiple
            hide-dropdown-icon
        ...
        <q-card-section v-show="false">
          <q-checkbox
            ...
            label="Agents hors ligne (Exécuter au programme contrôle)"
          >
            <q-tooltip
              >Si l'agent est hors ligne, une action en attente sera créée pour s'exécuter au contrôle suivant</q-tooltip
            >
          </q-checkbox>
        </q-card-section>
...
const monTypeOptions = [
  ...
  { label: "Serveurs", value: "servers" },
  { label: "Ordinateurs", value: "workstations" },
];
...
const targetOptions = [
  ...
  { label: "Agents Sélectionnés", value: "agents" },
  { label: "Tous", value: "all" },
];

const patchModeOptions = [
  { label: "Scan", value: "scan" },
  { label: "Installer", value: "install" },
];
