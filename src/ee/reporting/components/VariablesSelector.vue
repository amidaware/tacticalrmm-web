...
<template>
  ...
  <q-list v-else dense>
    <q-item-label header>Base Template Blocks</q-item-label>
    <q-item v-for="block in templateBlocks" :key="block" :inset-level="block.warning ? 0 : 1">
      <q-item-section avatar v-if="block.warning">
        <q-icon name="warning" color="warning">
          <q-tooltip>Block not found in template. Click on the block to copy and paste into template</q-tooltip>
        </q-icon>
      </q-item-section>
      <q-item-section>
        <span
    ...
    <q-item v-for="(type, prop) in variableAnalysis" :key="prop" @mouseover="mouseover = prop.toString()" @mouseleave="mouseover = ''">
      <q-item-section avatar>
      ...
      <q-item-section v-if="type.toLowerCase().substring(0,5) === 'array' && mouseover === prop.toString()" side>
...
function copy(content: string, is_for = false, block = false) {
  ...
  if (block) {
    ...
    text = "{% for item in " + content + " %}{% endfor %}"
  else
    text = "{{ " + content + " }}"
...
watchDebounced(() => props.variables, () => {
  getVariables()
},
{ debounce: 5000 })

// checks dependencies and adds warnings
function checkDependencies(dependsOn: string[] | undefined, dependencies: ReportDependencies | undefined) {
  dependencyWarnings.value = [];
    // Check if dependencies aren't specified
    dependsOn?.forEach((dep) => {
      !dependencies?.[dep] && dependencyWarnings.value.push(`Missing value for dependency: ${dep} . Open Preview to set values`);
    });
}
...
function checkBaseTemplate(template: string, base_id: number | undefined) {
  ...
  let regex = /\{% block ([A-Za-z0-9_ ]+) %\}/g, match: string[] | null

  if (base_template)
    while (match = regex.exec(base_template?.html)) {
      const full_match = match[0]
      const block_name = match[1]
      templateBlocks.value.push({block: block_name, warning: !template.includes(full_match)})
    }
}
}

// watches for changes in base template and template
watch([() => props.base_template, () => props.template], ([newBase, newTemplate]) => {
